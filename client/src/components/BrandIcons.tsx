...
// menu) and stays crisp at small sizes, matching the lucide-react icons used
// elsewhere. Third-party marks are the official single-path logos (simple-icons);
// the Athena mark mirrors src/assets/athena-mark.svg.

type BrandIconProps = SVGProps<SVGSVGElement> & { size?: number };

function FilledMark({ size = 14, path, viewBox = "0 0 24 24", label, ...rest }: BrandIconProps & { path: string; viewBox?: string; label: string }) {
  return (
    <svg
      width={size}
      height={size}
...
export function OpenAIIcon(props: BrandIconProps) {
  return <FilledMark {...props} path={OPENAI_PATH} label="Codex" />;
}

export function ClaudeIcon(props: BrandIconProps) {
  return <FilledMark {...props} path={CLAUDE_PATH} label="Claude" />;
}

export function OpenCodeIcon(props: BrandIconProps) {
  return <FilledMark {...props} path={OPENCODE_PATH} label="OpenCode" />;
}
