...
| Multi-workspace spawn resilience | P0 | Pending: make visible agent spawning faster and less fragile when several workspaces are open. Detect stale Electron control separately from FastAPI backend health; if `~/.context-workspace/electron-control.json` exists but `/health` fails, surface a clear stale-control state; safely self-heal/restart Electron control or remove/rewrite stale discovery files on boot/crash; consider watchdoging the Electron control server; speed up multi-agent spawning with batch/lower-level terminal spawn where appropriate, such as two OpenCode panes plus one Codex call; reduce nvm/`.npmrc` startup warnings; expose backend health and Electron-control health separately so “backend healthy” does not imply visible terminal spawning is healthy. |
